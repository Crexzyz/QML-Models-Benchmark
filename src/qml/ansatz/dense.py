...
class SingleAxisQCNNAnsatz(QCNNAnsatz):
    """
    Base class for single-axis QCNN ansatz variants.

    Uses only one rotation gate type (RX, RY, or RZ) for all six parametrized
    rotations inside each two-qubit convolution block.
    """

    def __init__(self, rotation_gate: Literal["rx", "ry", "rz"] = "ry"):
    ...
    def _apply_conv_block(self, params, wires) -> None:
        """Apply a single-axis two-qubit convolution block (6 params)."""
        self._gate_fn(params[0], wires=wires[0])
        self._gate_fn(params[3], wires=wires[1])
        qml.CNOT(wires=[wires[0], wires[1]])
        self._gate_fn(params[1], wires=wires[0])
        self._gate_fn(params[4], wires=wires[1])
        qml.CNOT(wires=[wires[0], wires[1]])
...
class SingleAxisQCNNAnsatz4(SingleAxisQCNNAnsatz):
    """
    Single-axis QCNN ansatz for 4 qubits with pooling.
    """

    def __call__(self, weights):
        """Apply the single-axis QCNN structure."""
        if len(weights) < self.n_layers:
            raise ValueError(
                f"Expected {self.n_layers} weight sets, got {len(weights)}"
            )
...
class SingleAxisQCNNAnsatz4NoPool(SingleAxisQCNNAnsatz):
    """
    Single-axis QCNN ansatz for 4 qubits without pooling.
    """

    def __call__(self, weights):
        """Apply the non-pooling single-axis QCNN structure."""
        if len(weights) < self.n_layers:
            raise ValueError(
                f"Expected {self.n_layers} weight sets, got {len(weights)}"
            )
...
class SingleAxisQCNNAnsatz8(SingleAxisQCNNAnsatz):
    """
    Single-axis QCNN ansatz for 8 qubits.
    """
