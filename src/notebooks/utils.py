...
def plot_loss_and_accuracy(losses, accuracies, test_losses=None, test_accuracies=None):
    ...
    line1 = ax1.plot(epochs, losses, color='tab:red', marker='o', label='Train Loss', linestyle='-')[0]
    lines = [line1]
    if test_losses is not None:
        line2 = ax1.plot(epochs, test_losses, color='tab:orange', marker='s', label='Test Loss', linestyle='--')[0]
        lines.append(line2)
    ...
    line3 = ax2.plot(epochs, accuracies, color='tab:blue', marker='x', label='Train Accuracy', linestyle='-')[0]
    lines.append(line3)
    if test_accuracies is not None:
        line4 = ax2.plot(epochs, test_accuracies, color='tab:cyan', marker='^', label='Test Accuracy', linestyle='--')[0]
        lines.append(line4)
    ...
    ax1.legend(lines, labels, loc='upper left', bbox_to_anchor=(0, -0.15), ncol=4, frameon=False)

    # Title and grid
    title = "Training and Test Metrics" if test_losses is not None else "Training Loss and Accuracy per Epoch"
    plt.title(title)
    fig.tight_layout()
